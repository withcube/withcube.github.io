...
function startTimer(keyCode) {
 if (timerStatus == 0 && manualEnter == 0 && keyCode == 32 && importFocus == 0) {
  ...
  if (inspection == 1) {
   ...
   startTime = new Date();
   penalty = 0;
   $('theTime').style.color = $('fcol').value;
...
function stopTimer(keyCode) {
 ...
 if (manualEnter == 1) {
  if (keyCode == 13) {
   ...
   if (timeStr.match(/.* .*/)) {
    if (parseTime(timeStr.replace(/(.*) .*/, "$1"))) { // if time breaks, ignore comments/notes
...
var scrdata = [["===WCA PUZZLES===",[["--","blank",0]]],
 ...
 ["3x3x3 subsets",[["2-generator <R,U>","2gen",25],["2-generator <L,U>","2genl",25],
  ...
  ["last slot + last layer","lsll2",0],["ZBLL","zbll",0],["2GLL","2gll",0],["PLL","pll",0],
